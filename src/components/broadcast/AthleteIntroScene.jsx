...
const AthleteIntroScene = ({
  ...
  return (
    ...
    >
      ...
      {/* ======================= [ Layer 5: 메인 뷰 — 세로형: BIO 좌측 + 인물 사진 우측 풀하이트 ] ======================= */}
      <div className={`relative z-10 w-full h-[calc(100vh-100px)] flex items-end px-4 pb-4 overflow-hidden`}>
        
        {/* -------------------- [ 우측: 메인 인물 사진 — 타원 마스킹으로 배경과 자연스럽게 블렌딩 ] -------------------- */}
        {heroPhoto && (
          <div className="absolute right-0 bottom-0 top-0 w-[58%] flex items-center justify-center z-10 pointer-events-none">
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
              <div className="w-[380px] h-[280px] bg-white/10 rounded-full blur-[90px] animate-pulse pointer-events-none" />
            </div>

            {/* 🖼️ 메인 인물 사진 — 타원형 마스킹으로 배경과 자연스럽게 녹아듦 */}
            <div
              ref={heroWrapperRef}
              className="relative h-[82vh] max-h-[820px] aspect-[3/4] flex items-center justify-center z-10"
              style={{
        ...
        {/* -------------------- [ 좌측: 선수 핵심 BIO 영역 ] -------------------- */}
        <div className={`space-y-4 w-full ${heroPhoto ? "max-w-[50%]" : "max-w-lg"} z-20 pb-4`}>
          
          {/* ① 배부번호 3D 스탬핑 */}
          <div
            ref={numberBadgeRef}
            className={`inline-flex items-center gap-3 bg-gradient-to-r ${theme.bgGradient} border-l-4 rounded-r-3xl pl-4 pr-6 ${theme.border} py-2 backdrop-blur-2xl shadow-2xl`}
          >
            ...
            <div className={`flex items-center font-mono font-black text-4xl sm:text-5xl tracking-tighter ${theme.primary} neon-number-glow`}>
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              {nameChars.map((char, i) => (
                <span
                  key={i}
                  className={`name-char inline-block text-6xl sm:text-7xl font-black tracking-tighter leading-none m-0 uppercase drop-shadow-[0_15px_40px_rgba(0,0,0,0.95)] ${theme.titleClass}`}
                >
                  {char}
                </span>
              ))}
          ...
          <div
            ref={gymBadgeRef}
            className="flex items-center gap-3 text-xl sm:text-2xl text-slate-100 font-black"
          >
            <div className={`p-2 rounded-2xl bg-white/10 border ${theme.border50} ${theme.primary} shadow-xl`}>
              <EnvironmentOutlined />
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 pt-1 w-full">
            {/* 1. HEIGHT */}
            <div className={`bio-card-item bg-black/80 backdrop-blur-2xl border ${theme.border40} rounded-2xl p-2.5 shadow-xl`}>
              <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">
                HEIGHT
              </span>
              <span className={`text-lg font-black font-mono ${theme.specText}`}>
                {heightValue}
              </span>
            </div>
